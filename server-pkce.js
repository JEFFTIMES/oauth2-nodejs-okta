...
const port = process.env.PORT_PKCE || 5000;

// configuration for communicating with the authorization server.
const authConfig = {
  response_type: 'code',
  client_id: process.env.CLIENT_ID_PKCE,
  ...
  requested_scopes: process.env.SCOPE
};
...
// loading the static page with given filename.
...
// generating code challenge string with SHA-256 hash algorithm and
// encoding it with base64url algorithm.
const pkceChallengeFromVerifier = function (verifier){

  // shajs hashing.
  //const digest = shajs('sha256').update(verifier).digest('base64');
  // crypto-js hashing.
  // const digest = SHA256(verifier).toString(CryptoJS.enc.Base64);
  
  // crypto hashing.
  const digest = crypto.createHash('sha256').update(verifier).digest('base64');
  // regexp replacing encoding.
  const cc = digest.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g,'');
  
  // base64url encoding.
  // const cc = base64url.fromBase64(digest);

  // console.log('encoded cc :', cc);
...
const composeAuthorizationURL = async function (config, localStor) {
  ...
  const verifier = await generateRadomString();
  ...
  localStor.verifier = verifier;
  localStor.codeChallenge = codeChallenge;
  ...
  const url = authConfig.authorization_endpoint 
        ...
        + "&code_challenge="+encodeURIComponent(localStor.codeChallenge)
...
router.route('/authorized').get(function (req, res, next){
  ...
  if(returnedUrl.searchParams.get('error')){
    console.log('authorization server return an error.')
    next(new Error(returnedUrl.searchParams.get('error')));
  }
  if(returnedUrl.searchParams.get('state') !== localStorage.state){
    console.log('authorization server returned a different state.')
  ...
  if(returnedUrl.searchParams.get('code')){
    ...
    // creating body string: grant_type=authorization_type&code=RETURNED_CODE&client_id=XXX&redirect_uri=XXX&code_verifier=XXX in the body.
    ...
    const callback = function (response) {
      //check status
      console.log('tk res status: ', response.statusCode);
      let responseBody = [];
      ...
      response.on('end', () => {
        ...
        if(response.statusCode === 200){
          ...
          localStorage.codeChallenge = '';
          console.log('token saved.');
          // directing user to the profile page.
          res.writeHead( 307, {'Location': `http://localhost:${port}/profile`});
          res.end();
    ...
    const tokenExchangeRequest = https.request(tokenEndpointUrl, options, callback);
